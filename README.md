...
- **高级筛选**: 包邮、验货宝、AI 个人卖家画像、新发布时间范围、省/市/区三级区域筛选
...
默认 `docker-compose.yaml` 会从当前源码构建本地镜像 `xianyu-tools:local`，以确保包含本仓库的企业微信应用消息、任务级通知目标、验货宝 / 包邮 / 个人卖家结果筛选等改动。

如果上游镜像无法访问或下载速度慢，可尝试使用加速：
```bash
...
- 本地默认镜像名：`xianyu-tools:local`
- 更新本地镜像：`docker compose up --build -d`
- 如需改用上游官方镜像 `ghcr.io/usagi-org/ai-goofish:latest`，请注意其中不一定包含本仓库的 fork-only 功能。
...
适用于从上游镜像或旧版 fork 升级到当前 `xianyu-tools` 本地源码部署。更详细的验证流程见 `docs/runbooks/local-smoke.md`，当前批次 PR 摘要见 `docs/pr-draft-parallel-refactor-batch.md`。

1. 更新代码后先确认 `docker-compose.yaml` 仍从当前源码构建本 fork 镜像：默认镜像名为 `xianyu-tools:local`，不要误切回上游 `ghcr.io/usagi-org/ai-goofish:latest`，否则企业微信应用通知、任务级通知目标、验货宝/包邮/个人卖家筛选等 fork-only 功能可能缺失。
...
4. SQLite 迁移是启动时自动执行的 additive migration：当前批次只会用 `ALTER TABLE ... ADD COLUMN` 为 `result_items` 增加 `is_processed` / `is_contacted`（P3-2）默认字段，不会删除或重写旧数据。M9-2 的卖家点击筛选是无 schema 的 query/filter 改动。升级前仍建议备份 `data/app.sqlite3`。
...
- 任务表单按基础信息、判断模式、搜索筛选、定时与账号、通知目标分区折叠，降低长表单干扰。
- 支持 AI 创建、关键词规则、价格范围、新发布范围、包邮、验货宝、区域筛选、账号绑定、定时规则；`yhb_only` 会在任务保存、运行配置和闲鱼搜索过滤中保持一致。
...
- 结果页和 CSV 导出支持按验货宝、包邮、仅看个人卖家筛选；“仅看个人卖家”基于 AI 输出的 `seller_type` 画像判断，缺少该画像的关键词结果或旧结果不会被匹配。
...
- `npm run build` 会通过 Vite 配置直接生成仓库根目录 `dist/`。
...
说明根目录 `dist/` 缺失。可直接执行 `./start.sh`，或先在 `web-ui/` 里执行 `npm run build`，再确认构建产物已生成到仓库根目录。
