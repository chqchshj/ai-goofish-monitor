...
- **高级筛选**: 包邮、验货宝、新发布时间范围、省/市/区三级区域筛选
...
默认 `docker-compose.yaml` 会从当前源码构建本地镜像 `xianyu-tools:local`，以确保包含本仓库的企业微信应用消息和任务级通知目标等改动。
...
- 支持 AI 创建、关键词规则、价格范围、新发布范围、包邮、验货宝、区域筛选、账号绑定、定时规则。
...
- 结果页支持按验货宝和包邮筛选；未提供个人卖家筛选，因为当前保存的结果数据没有可靠字段可判断个人卖家。
