...
- 决策入口：`evaluate_notification(record, policy, dedup_store, now)` —— 纯函数，决定一条已被 AI 标记 `is_recommended=True` 的记录是否真的发送通知。
...
- 切换平滑：`evaluate_notification` 在 inert 策略下仍会 `dedup_store.mark`，避免运维启用窗口的瞬间把历史 key 全部重发。
